...
/**
 * Microsoft SQL server database type information used to create the tables,
 * etc..
 * 
 * <p>
 * <b>WARNING:</b> I have not tested this unfortunately because of a lack of
 * permanent access to a MSSQL instance.
 * </p>
 * 
 * @author graywatson
 */
public class SqlServerDatabaseType extends BaseDatabaseType
{
	...
	@Override
	protected void appendUuidNativeType(StringBuilder sb,
										FieldType fieldType,
										int fieldWidth)
	{
	...
	{
		/*
		 * The only way sql-server allows this is if
		 * "SET IDENTITY_INSERT table-name ON" has been set. However this is a
	...
	/**
	 * Conversion from the byte Java field to the SMALLINT Jdbc type because
	 * TINYINT looks to be 0-255 and unsigned.
	 */
	private static class ByteFieldConverter extends BaseFieldConverter
	{
		...
		{
			// starts as a short and then gets converted to a byte on the way
			// out
